...
class FastTranslationDataset(Dataset):
    ...
    def __init__(self, cache_path, fields, subsampling=False, num_samples = 1000, **kwargs):
        ...
        if subsampling:
            ...
            cached_data_src = cached_data_src[:num_samples]
            cached_data_trg = cached_data_trg[:num_samples]
...
class DatasetWrapper(FastTranslationDataset):
    ...
    @classmethod
    def get_train_and_val_datasets(cls, train_cache_path, val_cache_path, fields, subsampling=False, num_samples=1000, **kwargs):

        train_dataset = cls(train_cache_path, fields, subsampling=subsampling, num_samples=num_samples, **kwargs)
...
def get_datasets_and_vocabs(dataset_path, language_direction, use_iwslt=True, use_caching_mechanism=True, subsampling=False, num_samples=1000, ood=False):
    ...
    if not ood:
        ...
        ### NEW CASE ADDED HERE ###
        # If loading an OOD dataset, build the vocabulary from the ID dataset.
        id_train_dataset, id_val_dataset, _, _ = get_datasets_and_vocabs(
            dataset_path=dataset_path,
            language_direction=language_direction,
            use_iwslt=not use_iwslt,
            use_caching_mechanism=use_caching_mechanism,
            subsampling=True,
            num_samples=num_samples,
            ood=False,
        )
        src_field_processor.build_vocab(id_train_dataset.src, min_freq=MIN_FREQ)
        trg_field_processor.build_vocab(id_train_dataset.trg, min_freq=MIN_FREQ)
...
def get_data_loaders(dataset_path, language_direction, dataset_name, batch_size, device, subsampling=False, num_samples=1000, ood=False):
