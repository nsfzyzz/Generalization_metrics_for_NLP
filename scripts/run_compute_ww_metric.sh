...
do
    ...
    do
        for lr in 0.125 0.25
	do
        # order is [checkpoint dir] [result_dir] [num_samples] [depth]
        #sbatch scripts/compute_ww_metric_PL.sh \
        sbatch scripts/compute_ww_metric.sh "/work/yyaoqing/Good_vs_bad_data/checkpoint/NMT_epochs/Simpson/WMT14_sample${samples}_depth${depth}_lr${lr}_dropout0.1" \
