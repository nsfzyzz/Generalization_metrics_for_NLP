...
def get_metric_bleu_df(experiment, distribution, adjust_measures_back, this_metric, bleu_from_logs=False):
    ...
    for metric, metric_file in METRIC_FILES.items():
        if metric != this_metric:   # We only need to plot one metric
            continue
    ...
    if bleu_from_logs:
        ...
        FILE_TRAIN = os.path.join(experiment, "bleu_loss.jsonl")
        if not os.path.exists(FILE_TRAIN):
            FILE_TRAIN = os.path.join(experiment, "bleu_loss_only_train.jsonl")
        ...
        with open(FILE, "rb") as file:
            for line in file:
                ...
                id_bleu_gaps.append(id_bleu_train_scores[EPOCH-1] - d[f'epoch{EPOCH}_id_bleu_score']* 100)
                EPOCH += 1
    else:
        EPOCH = 1   # Epochs are numbered 1-20
        FILE = os.path.join(experiment, "bleu_loss.jsonl")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--bleu_from_logs", dest='bleu_from_logs', action='store_true', help='get the BLEU from the log file')
    ...
    elif plot_metric_name == 'exp_dist_exponent':
        plot_metric_name = 'EXP_lambda'
